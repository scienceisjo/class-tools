...
| 자비스 | 수업 시작에 3~5분 재미로 보여주는 아이언맨 JARVIS 데모. **손뼉 두 번**에 부팅해 "Welcome home, sir."(영어 음성 + 한글 자막). 음성으로 **음악**·날씨·시간·시스템 진단(실제 하드웨어 값)·타이머·브리핑을 부르고, **오늘의 학습목표**는 해독·스캔·조립·고정 중 매번 다른 연출로 공개 — `[대괄호]`로 감싼 낱말은 빈칸 퀴즈가 되어 아이들이 맞힌 뒤에 열립니다. 교실 소음으로 음성이 안 먹혀도 숫자키로 똑같이 조작되는 키보드 백도어 내장. 마이크가 필요하므로 크롬·엣지에서 열 것. | [jarvis/](jarvis/) |
