...
모두 브라우저에서 바로 여는 단일 HTML 파일이며, 전자칠판에서 사용하기 좋게 만들었습니다.
...
- **🍱 오늘 급식(나이스 위젯 + 만드는 법):** https://scienceisjo.github.io/class-tools/lunch/
...
| 자비스 | 수업 시작에 3~5분 재미로 보여주는 아이언맨 JARVIS 데모. **손뼉 두 번**에 부팅해 "Welcome home."(영어 음성 + 한글 자막). 음성으로 **음악**·날씨·시간·시스템 진단(실제 하드웨어 값)·타이머·브리핑·오늘의 명언(읽어주기)·**급식**(나이스)·**발표자/모둠 뽑기**(도는 동안 숫자키로 몰래 정하기)·**생일 축하**·**소음 신호등**을 부르고, **오늘의 학습목표**는 해독·스캔·조립·고정 중 매번 다른 연출로 공개 — `[대괄호]`로 감싼 낱말은 빈칸 퀴즈가 되어 아이들이 맞힌 뒤에 열립니다. **어느 선생님이든 그대로 쓸 수 있습니다**: `Ctrl+Alt+J` 로 열리는 숨은 **교사 콘솔**에서 이름·호칭·자비스 대사·학습목표·음악·브리핑을 모두 바꾸고, 설정을 파일로 내보내 동료에게 건넬 수 있어요. 교실 소음으로 음성이 안 먹혀도 숫자키로 똑같이 조작되는 키보드 백도어 내장. 마이크가 필요하므로 크롬·엣지에서 열 것. | [jarvis/](jarvis/) |
| 오늘 급식 | 학교 이름만 넣으면 이번 주 급식이 뜨는 위젯(나이스 교육정보 개방포털, 키 없음·서버 없음). 학교가 주소에 들어가 학급 홈에 붙이기 좋고, 아래에 **급식 API를 직접 가져오는 법**과 우리 학교 코드가 채워진 **바이브코딩 프롬프트**, 알레르기 번호표가 있어 연수 자료로도 씀. | [lunch/](lunch/) |
